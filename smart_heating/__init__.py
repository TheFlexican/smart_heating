...
async def async_setup_services(hass: HomeAssistant, coordinator: SmartHeatingCoordinator) -> None:
    ...
    hass.services.async_register(
        DOMAIN, 
        SERVICE_REFRESH, 
        partial(async_handle_refresh, coordinator=coordinator)
    )
    hass.services.async_register(
        DOMAIN, 
        SERVICE_ADD_DEVICE_TO_AREA, 
    ...
    hass.services.async_register(
        DOMAIN, 
        SERVICE_SET_OPENTHERM_GATEWAY, 
        partial(async_handle_set_opentherm_gateway, area_manager=area_manager, coordinator=coordinator), 
        schema=OPENTHERM_GATEWAY_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, 
        SERVICE_SET_TRV_TEMPERATURES, 
        partial(async_handle_set_trv_temperatures, area_manager=area_manager, coordinator=coordinator), 
    ...
    hass.services.async_register(
        DOMAIN, 
        SERVICE_SET_FROST_PROTECTION, 
        partial(async_handle_set_frost_protection, area_manager=area_manager, coordinator=coordinator), 
    ...
    hass.services.async_register(
        DOMAIN, 
        SERVICE_REMOVE_WINDOW_SENSOR, 
        partial(async_handle_remove_window_sensor, area_manager=area_manager, coordinator=coordinator), 
        schema=WINDOW_SENSOR_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, 
        SERVICE_ADD_PRESENCE_SENSOR, 
        partial(async_handle_add_presence_sensor, area_manager=area_manager, coordinator=coordinator), 
        schema=PRESENCE_SENSOR_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, 
        SERVICE_REMOVE_PRESENCE_SENSOR, 
        partial(async_handle_remove_presence_sensor, area_manager=area_manager, coordinator=coordinator), 
        schema=PRESENCE_SENSOR_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, 
    ...
    hass.services.async_register(
        DOMAIN, 
        SERVICE_SET_SAFETY_SENSOR, 
        partial(async_handle_set_safety_sensor, hass=hass, area_manager=area_manager, coordinator=coordinator), 
        schema=SAFETY_SENSOR_SCHEMA
    )
    hass.services.async_register(
        DOMAIN, 
        SERVICE_REMOVE_SAFETY_SENSOR, 
        partial(async_handle_remove_safety_sensor, hass=hass, area_manager=area_manager, coordinator=coordinator)
    )
