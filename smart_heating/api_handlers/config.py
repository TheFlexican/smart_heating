...
async def handle_set_global_presets(area_manager: AreaManager, data: dict) -> web.Response:
    ...
    if "activity_temp" in data:
        ...
        _LOGGER.warning("  Global Activity: %.1f°C → %.1f°C", old, area_manager.global_activity_temp)
...
async def handle_get_hysteresis(area_manager: AreaManager) -> web.Response:
    ...
    """
    return web.json_response({
        "hysteresis": area_manager.hysteresis
    })
...
async def handle_get_global_presence(area_manager: AreaManager) -> web.Response:
    ...
    """
    return web.json_response({
        "sensors": area_manager.global_presence_sensors
    })
...
async def handle_set_global_presence(area_manager: AreaManager, data: dict) -> web.Response:
    ...
    if "sensors" in data:
        ...
        _LOGGER.warning("  Global presence sensors updated: %d sensors", len(area_manager.global_presence_sensors))
...
async def handle_set_safety_sensor(
    ...
    hass.bus.async_fire(
        ...
        {"sensor_id": sensor_id, "enabled": True}
    )
...
async def handle_remove_safety_sensor(hass: HomeAssistant, area_manager: AreaManager) -> web.Response:
