...
async def handle_get_efficiency_report(
    ...
    try:
        ...
        if area_id and area_id != "all":
            ...
            # All areas report
            area_reports_raw = (
                await efficiency_calculator.calculate_all_areas_efficiency(
                    area_manager, period
                )
            )
                ...
                area_reports.append(
                    {
                        ...
                        "metrics": {
                            ...
                            "heating_time_percentage": raw_report.get(
                                "heating_time_percentage", 0
                            ),
                            "heating_cycles": raw_report.get("heating_cycles", 0),
                            "avg_temp_delta": raw_report.get(
                                "average_temperature_delta", 0
                            ),
                        },
            ...
            if area_reports_raw:
                total_energy_score = sum(
                    r.get("energy_score", 0) for r in area_reports_raw
                )
